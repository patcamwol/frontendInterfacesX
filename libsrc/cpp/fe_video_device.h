 ...
 */
#ifndef FE_VIDEO_DEVICE_BASE_H
#define FE_VIDEO_DEVICE_BASE_H
...
namespace frontendX {
    ...
    /*
     *    Frontend video class definition
     */
    template < typename VideoStatusStructType >
    class FrontendVideoDevice : public Device_impl
    {
        ENABLE_LOGGING

        public:
            FrontendVideoDevice(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl);
            FrontendVideoDevice(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, char *compDev);
            FrontendVideoDevice(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities);
            FrontendVideoDevice(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev);
            ~FrontendVideoDevice();
        ...
        protected:
            ...
            frontend_video_allocation_struct frontend_video_allocation;
            frontend::frontend_listener_allocation_struct frontend_listener_allocation;
            std::vector<VideoStatusStructType> frontend_video_status;

            // video_device_allocation_ids is exclusively paired with property frontend_video_status.
            // video_device_allocation_ids tracks allocation ids while frontend_video_status provides video_device information.
            std::vector<frontend::tunerAllocationIdsStruct> video_allocation_ids;

            // Provides mapping from unique allocation ID to internal video device number
            string_number_mapping allocation_id_to_video_device_id;
            boost::mutex allocation_id_mapping_lock;

            ///////////////////////////////
            // Device specific functions // -- virtual - to be implemented by device developer
            ///////////////////////////////
            virtual void videoDeviceEnable(VideoStatusStructType &fts, size_t video_device_id) = 0;
            virtual void videoDeviceDisable(VideoStatusStructType &fts, size_t video_device_id) = 0;
            virtual bool videoDeviceSetTuning(const frontend_video_allocation_struct &request, VideoStatusStructType &fts, size_t video_device_id) = 0;
            virtual bool videoDeviceDeleteTuning(VideoStatusStructType &fts, size_t video_device_id) = 0;

            ///////////////////////////////
            // Mapping and translation helpers. External string identifiers to internal numerical identifiers
            ///////////////////////////////
            virtual std::string getControlAllocationId(size_t video_device_id);
            virtual std::vector<std::string> getListenerAllocationIds(size_t video_device_id);
            virtual long getVideoDeviceMapping(std::string allocation_id);
            std::string createAllocationIdCsv(size_t video_device_id);
            virtual bool removeVideoDeviceMapping(size_t video_device_id, std::string allocation_id);
            virtual bool removeVideoDeviceMapping(size_t video_device_id);
            ...
            virtual void removeAllocationIdRouting(const size_t video_device_id) = 0;
            virtual void setNumChannels(size_t num) = 0;

            // Configure video_device - gets called during allocation
            virtual bool enableVideoDevice(size_t video_device_id, bool enable);
            virtual bool listenerRequestValidation(frontend_video_allocation_struct &request, size_t video_device_id);


            ////////////////////////////
            // Other helper functions //
            ////////////////////////////
            BULKIO::StreamSRI create(std::string &stream_id, VideoStatusStructType &frontend_status) {
                BULKIO::StreamSRI sri;
                sri.hversion = 1;
                sri.xstart = 0.0;
                if ( frontend_status.fps <= 0.0 )
                    sri.xdelta =  1.0;
                else
                    sri.xdelta = 1/frontend_status.fps;
